...
export ADEMC, CreateADEMCTestFunc, ADEMCTestFuncMarginalPDF

@doc raw"""
    ADEMCSampler(lprobFunc::Function, init::Array, niter::Int; sigma::Float64=1e-5, gamma=nothing, aimh_prob::Float64=0.05, nsamples_proposal_dist=nothing, df_proposal_dist::Int=10, progress::Bool=true)
...
- `nsamples_proposal_dist::Int=nothing`: the window size used to calculate the rolling-window covariance estimate. By default this is the number of unique elements in the proposal mean and covariance ``0.5 d(d+3)``.
- `df_proposal_dist::Float=10`: the degrees of freedom of the multivariate t distribution used for AIMH proposals.
"""
function ADEMC(lprobFunc::Function, init::Array, niter::Int; sigma::Float64=1e-5, gamma=nothing, aimh_prob::Float64=0.05, nsamples_proposal_dist=nothing, df_proposal_dist::Int=10, progress::Bool=true)

    ndim, nchain = size(init)

    # get some default values
    npdist = nsamples_proposal_dist
    ...
    if nsamples_proposal_dist == nothing
        npdist = Int(0.5 * ndim * (ndim + 3))
    else
        npdist = nsamples_proposal_dist
    end
...
@doc raw"""
    CreateADEMCTestFunc(ndim::Int, weight::Float, distance::Float, scale::Float)

Create a bimodal Gaussian mixture for testing.
"""
function CreateADEMCTestFunc(ndim, weight, distance, scale)
...
"""
function ADEMCTestFuncMarginalPDF(x, cov_scale, distance, weight)
